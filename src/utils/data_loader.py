...
def multiple_aggregate_by_year(
...
def top_name_by_genre(dataset: pd.DataFrame) -> Dict[str, int | str]:
    ...
    pivot = (
        df.pivot(index="pr1", columns="genre", values="count")
        .rename(columns={"F_minin": "Féminin", "Ind_termin_": "Indéterminé"})
        .fillna(0)
    )

    return {"male": pivot["Masculin"].idxmax(), "female": pivot["Féminin"].idxmax()}
...
def top_name_by_year_and_gender(dataset: pd.DataFrame) -> pd.DataFrame:
    ...
    result = result.rename(columns={"F_minin": "Féminin"})
    return result
...
def aggregate_by_gender_and_by_year(dataset: pd.DataFrame) -> pd.DataFrame:
    ...
    df_pivot = (
        df.pivot(index="annee", columns="genre", values="count")
        .rename(columns={"F_minin": "Féminin", "Ind_termin_": "Indéterminé"})
        .fillna(0)
    )
...
def top_or_down_birth(dataset: pd.DataFrame) -> Dict[str, Dict]:
    ...
    return {
        "all": {
            "highest_year": {
                "year": int(highest_year),
                "value": int(df.loc[highest_year, "total"]),
            },
            "lowest_year": {
                "year": int(lowest_year),
                "value": int(df.loc[lowest_year, "total"]),
            },
        },
        "female": {
            "highest_year": {
                "year": int(highest_female),
                "value": int(df.loc[highest_female, "Féminin"]),
            },
            "lowest_year": {
                "year": int(lowest_female),
                "value": int(df.loc[lowest_female, "Féminin"]),
            },
        },
        "male": {
            "highest_year": {
                "year": int(highest_male),
                "value": int(df.loc[highest_male, "Masculin"]),
            },
            "lowest_year": {
                "year": int(lowest_male),
                "value": int(df.loc[lowest_male, "Masculin"]),
            },
        },
    }
...
def top_year_and_down_year(dataset: pd.DataFrame) -> Dict:
    ...
    return {
        "highest_year": {
            "year": int(highest_year),
            "value": int(series.loc[highest_year]),
        },
        "lowest_year": {
            "year": int(lowest_year),
            "value": int(series.loc[lowest_year]),
        },
    }
...
def _age_of_death(dt: pd.DataFrame) -> pd.DataFrame:
    dataset = dt.copy()
    dataset["date_naissance"] = pd.to_datetime(
        dataset["date_naissance"], errors="coerce", dayfirst=True
    )
...
def death_age_histogram(dt: pd.DataFrame, bins: int = 20, year_range=None) -> pd.Series:
    ...
    histogram.index = [
        f"{int(interval.left)}-{int(interval.right)}" for interval in histogram.index
...
def death_by_day(dt: pd.DataFrame) -> pd.Series:
    ...
    death_d_df = df.groupby("jour_num").size().reindex(range(7), fill_value=0)
