from .data_loader import (dataset_load, size_dataset,
                          aggregate_by_year, multiple_aggregate_by_year, aggregate_by_gender, top_name,
                          _aggregate_by_name, aggregate_by_gender_and_by_year, top_or_down_birth,
                          top_name_by_genre, top_name_by_year_and_gender, find_name_query, name_vs_name,
                          top_and_down_death_year,
                          average_death_age_by_year,
                          death_age_histogram, average_death_age_by_year_and_genre,
                          death_by_season_month, death_by_day, death_by_month_chart)